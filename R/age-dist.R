...
rosters %>%
  ...
  ungroup() %>%
ggplot() +
  stat_density_ridges(aes(age, fct_reorder(toupper(country), -age, median)),
    quantile_lines = TRUE, quantiles = 2,
    scale = 0.9, color = "#e7051a", fill = "#f7ca07") +
  scale_x_continuous(expand = c(0, 0), limits = c(18, 42), breaks = seq(18, 42, 4)) +
  coord_cartesian(clip = "off") +
  labs(title = "Age distribution in the FIBA Basketball World Cup 2019",
    subtitle = "By team, sorted by median age",
  ...
  ggsave(here::here("figures", "age_dist.png"),
